...
public class RMListScenario extends TafTestBase {
    ...
    @TestId(id = "TORF-47090_list_all", title = "Role Management UI - List All Roles")
    @Test(groups = {"RFA"})
    public void testRolesList() throws Exception {
        final TestScenario scenario = scenario("Role Management List scenario")
                .addFlow(loginLogoutFlow.loginWithRedirect(REDIRECT_ROLE_PAGE))
                .addFlow(rolesListFlow.verifyRolesAreDisplayedCorrectly(CSV_TEST_ROLES))
