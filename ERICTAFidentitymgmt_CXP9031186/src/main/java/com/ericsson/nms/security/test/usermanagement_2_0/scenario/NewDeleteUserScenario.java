...
public class NewDeleteUserScenario extends TafTestBase {
    ...
    @Test(groups = { "Acceptance", "RFA", "RFA250", "Delete User" })
    @TestSuite
    public void testCreateEditAndDeleteUser() throws Exception {
        ...
        final TestScenario scenario = dataDrivenScenario("Delete User Scenario")
                .withScenarioDataSources(dataSource(TEST_DS).withFilter(PredicatesExt.testCaseIdFilter("42")))
                .addFlow(loginLogoutFlow.loginWithRedirect(USER_MGMT_APP_NAME_FILTER))
                .addFlow(deleteUserFlow.deleteUserOnList())
    ...
    public void tearDown() {
        context.addDataSource(USER_TO_CLEAN_UP, TafDataSources.combine(
                TafDataSources.fromCsv(CREATE_USER_CSV),
                TafDataSources.fromCsv(CREATE_USER_FOR_DELETE_ON_EDIT_CSV)));
